  ...
  return (
    <section>
      <section style={{ marginBottom: 18 }}>
        <h4 style={{ margin: '0 0 8px 0' }}>Propositions de délégation</h4>
        <div style={{ display: 'grid', gap: 8 }}>
          {delegations.length === 0 && <div style={{ color: '#6b7280' }}>Aucune proposition</div>}
          {delegations.map((d) => (
            <div key={d.id} role="button" tabIndex={0} onClick={() => openDelegationDetails(d)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openDelegationDetails(d) } }} style={{ background: '#fff', padding: 10, borderRadius: 8, boxShadow: '0 1px 2px rgba(0,0,0,0.04)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}>
              <div>
                <div style={{ fontWeight: 700 }}>{d.id} — {d.statut}</div>
                <div style={{ color: '#374151', marginTop: 6 }}>Affectation: {d.affectationOriginale} • Proposé à: {d.auditeurPropose}</div>
                <div style={{ color: '#6b7280', marginTop: 6 }}>{d.justification || '-'}</div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={(e) => { e.stopPropagation(); const aff = tasks.find(t => t.id === d.affectationOriginale); if (aff) { openDetails(aff.tacheId) } else { alert('Tâche introuvable') } }} style={{ padding: '6px 10px', borderRadius: 8, border: 'none', background: '#2563eb', color: '#fff' }}>Voir tâche</button>
                {d.statut === 'EN_ATTENTE' && (
                  <>
                    <button onClick={async (e) => { e.stopPropagation(); setActionLoading(d.id); try { await acceptDelegation(d.id); await getAffectations().then((r)=>setTasks(r)); await loadDelegations(); } catch(err){console.error(err)} finally{setActionLoading(null)} }} disabled={actionLoading === d.id} style={{ background: '#10b981', color: '#fff', border: 'none' }}>Accepter</button>
                    <button onClick={async (e) => { e.stopPropagation(); setActionLoading(d.id); try { await refuseDelegation(d.id); await loadDelegations(); } catch(err){console.error(err)} finally{setActionLoading(null)} }} disabled={actionLoading === d.id} style={{ background: '#ef4444', color: '#fff', border: 'none' }}>Refuser</button>
                  </>
                )}
              </div>
            </div>
...
<article
            key={t.id}
            style={{ ...cardStyle, display: 'flex', flexDirection: 'column', height: '100%', cursor: 'pointer' }}
            tabIndex={0}
            role="button"
            onClick={() => openDetails(t.tacheId)}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openDetails(t.tacheId); } }}
          >
            <div style={{ marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <strong style={{ fontSize: 16 }}>Affectation: {t.id}</strong>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ color: '#6b7280', fontSize: 12 }}>{t.dateAffectation}</div>
                  <div style={{ fontSize: 12 }}>{t.mode} • {t.statut}</div>
                </div>
              </div>
              <div style={{ color: '#374151', marginTop: 8 }}>
                Tâche: {t.tacheId} • Auditeur: {t.auditeurId}
              </div>
              <div style={{ color: '#6b7280', marginTop: 8, fontSize: 13 }}>
                {t.justificatifRefus ? 'Justif: ' + t.justificatifRefus : ''}
              </div>
            </div>

                  <div style={{ marginTop: 'auto', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                    <button disabled={actionLoading === t.id} style={{ background: '#10b981', color: '#fff', border: 'none' }} onClick={(e) => { e.stopPropagation(); openModal('accept', t.id); }}>{actionLoading === t.id ? '…' : 'Accepter'}</button>
                    <button disabled={actionLoading === t.id} style={{ background: '#ef4444', color: '#fff', border: 'none' }} onClick={(e) => { e.stopPropagation(); openModal('refuse', t.id); }}>{actionLoading === t.id ? '…' : 'Refuser'}</button>
                    <button disabled={actionLoading === t.id} style={{ background: '#f59e0b', color: '#fff', border: 'none' }} onClick={(e) => { e.stopPropagation(); openDelegateModal(t.id); }}>{actionLoading === t.id ? '…' : 'Déléguer'}</button>
                  </div>
          </article>
        ))}
      </div>
